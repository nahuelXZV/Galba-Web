<div class="mt-5">
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li aria-current="page">
                <div class="flex items-center">
                    <x-iconos.flecha />
                    <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2 dark:text-gray-500">Docentes</span>
                </div>
            </li>
        </ol>
    </nav>
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
            ...
            <tbody>
                @foreach ($docentes as $docente)
                    <tr
                        ...
                        <td class="px-6 py-4">
                            {{ $docente->honorifico }} {{ $docente->nombre }} {{ $docente->apellido }}
                        </td>
                        <td class="px-6 py-4">
                            {{ $docente->correo }}
                        </td>
                        <td class="px-6 py-4">
                            {{ $docente->ci }} {{ $docente->expedido }}
                        </td>
                        <td class="px-6 py-4">
                            {{ $docente->telefono }}
                        </td>
                        <td class="px-6 py-4 text-right">
                            ...
                            <button type="button" wire:click="delete({{ $docente->id }})"
        ...
        <x-shared.pagination :modelo='$docentes' />
    </div>
